...
export default function DonationOverviewPage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user && !hasWithdrawalDataSyncedRef.current) {
      // 确保用户对象中存在这些属性
      if (
        user.withdrawAmount !== undefined ||
        user.rewardAmount !== undefined
      ) {
        // 计算withdrawnAmount和withdrawableAmount
        const withdrawnAmount = user.withdrawAmount
          ? parseFloat(user.withdrawAmount)
          : 0;
  ...
  useEffect(() => {
    ...
    if (
      !hasDataSyncedRef.current &&
      teamInfo &&
      teamInfo.totalReferrals !== undefined
    ) {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && mounted && !hasProfitDataLoadedRef.current) {
      ...
      getUserProfit()
        ...
        .catch((err) => {
          console.error(
            "[DonationOverviewPage] Error fetching profit data:",
            err
          );
        });
    }
        ...
        <div className="max-w-lg mx-auto">
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-bold text-[#d4b96e]">
              {t("donationOverview.myDonations")}
            </h1>
            ...
            >
              {isRefreshing ? (
                <>
                  <svg
                    className="animate-spin -ml-1 mr-2 h-4 w-4 text-[#d4b96e]"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <DonationOverview
          data={{
            ...
            withdrawnAmount: user?.withdrawAmount
              ? parseFloat(user.withdrawAmount)
              : donationData.withdrawnAmount,
            withdrawableAmount: user?.rewardAmount
              ? parseFloat(user.rewardAmount)
              : donationData.withdrawableAmount,
            dailyFunds: {
        ...
        <div className="mt-6 p-5 rounded-xl bg-[#1a0d2c]/90 backdrop-blur-sm">
          <h2 className="text-lg font-bold text-[#d4b96e] mb-4">
            {t("donationOverview.donationDetails")}
          </h2>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-islamic-medium/30 pb-3">
              <span className="text-sm text-islamic-cream/80">
                {t("donationOverview.donationDate")}
              </span>
            ...
            <div className="flex justify-between items-center border-b border-islamic-medium/30 pb-3">
              ...
              <span className="text-sm font-medium text-islamic-cream flex items-baseline">
                {donationData.totalDonation || 0}
            ...
            <div className="flex justify-between items-center border-b border-islamic-medium/30 pb-3">
              <span className="text-sm text-islamic-cream/80">
                {t("donationOverview.totalReferrals")}
              </span>
              <span className="text-sm font-medium text-islamic-cream">
                {teamInfo?.totalReferrals || 0}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center">
              <span className="text-sm text-islamic-cream/80">
                {t("donationOverview.transactionId")}
              </span>
              <span className="text-sm font-medium text-islamic-cream">
                TX123456789
              </span>
