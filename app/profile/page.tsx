      ...
      getUserProfit()
        .then((res) => {
          if (res.success && res.data) {
            ...
            console.error(
              "[ProfilePage] Failed to get profit data:",
              res.error
            );
          }
        })
    ...
    <MainLayout title={t("profile.title")} currentPath="/profile">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            <div className="bg-[#1a1f2c] p-3 rounded-lg text-center">
              ...
              <p className="text-lg font-bold text-[#d4b96e]">
                {isDataLoading ? "..." : displayData.totalDonation}{" "}
                <span className="text-sm">{t("units.u")}</span>
              </p>
            </div>
            <div className="bg-[#1a1f2c] p-3 rounded-lg text-center">
              <p className="text-xs text-islamic-cream/70 mb-1">
                {t("profile.reliefFunds")}
              </p>
              <p className="text-lg font-bold text-[#8dc63f]">
                {isDataLoading ? "..." : displayData.reliefFunds}{" "}
                <span className="text-sm">{t("units.u")}</span>
              </p>
            </div>
            <div className="bg-[#1a1f2c] p-3 rounded-lg text-center">
            ...
            <div className="mt-3 grid grid-cols-2 gap-4">
              <div>
                <p className="text-xs text-islamic-cream/70">
                  {t("donation.totalDonationAmount")}
                </p>
                <p className="text-lg font-bold text-[#d4b96e]">
                  {displayData.totalDonation}{" "}
                  <span className="text-sm">{t("units.u")}</span>
                </p>
                <p className="text-xs text-islamic-cream/70 mt-1">
                  {t("profile.vipLevel")} {displayData.vipLevel}
                </p>
              </div>
              <div>
                <p className="text-xs text-islamic-cream/70">
                ...
                <p className="text-lg font-bold text-[#8dc63f]">
                  {donationData?.dailyFunds?.current || 0}-
                  {donationData?.dailyFunds?.max || 0}{" "}
                  <span className="text-sm">{t("units.u")}</span>
                </p>
                <p className="text-xs text-islamic-cream/70 mt-1">
                  {displayData.referrals} {t("profile.referrals")}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <div className="space-y-4">
        <Card className="border-[#d4b96e]/20 bg-[#131b29]/80 backdrop-blur-sm overflow-hidden">
          <CardContent className="p-4">
            <h3 className="text-base font-medium text-[#d4b96e] mb-3">
              {t("profile.myTeam")}
            </h3>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-medium">
                  {teamData.directReferrals || 0} {t("profile.people")}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm">{t("profile.totalTeamSize")}</span>
                <span className="text-sm font-medium">
                  {teamData.totalReferrals || 0} {t("profile.people")}
                </span>
              </div>
              <div className="flex justify-between items-center">
        ...
        <Card className="border-[#d4b96e]/20 bg-[#131b29]/80 backdrop-blur-sm overflow-hidden">
          <CardContent className="p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm">
                  {t("profile.accumulatedReliefFunds")}
                </span>
                <span className="text-sm font-medium text-[#8dc63f]">
                  {displayData.reliefFunds}{" "}
                  <span className="text-xs">{t("units.u")}</span>
                </span>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm">
                  {t("profile.availableToWithdraw")}
                </span>
                <span className="text-sm font-medium text-[#8dc63f]">
                  {displayData.withdrawable}{" "}
                  <span className="text-xs">{t("units.u")}</span>
                </span>
              </div>
            </div>
