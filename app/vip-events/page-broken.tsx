...
// import { overseasCountriesData } from "@/lib/overseas-data";
import {
  Play,
  Calendar,
  MapPin,
...
export default function VipEventsPage() {
  ...
  const handleImageError = (
    ...
    target.innerHTML = `<div class="flex flex-col items-center justify-center w-full h-full">
      ...
      <span class="text-xs text-[#d4b96e]">Loading...</span>
    </div>`;
  };
        ...
        <div className="mt-2">
          ...
          {selectedCategory === "overseas" && (
            <div className="space-y-6">
              ...
              {/* 海外务工国家列表 */}
              <div className="space-y-4">
                <div className="text-center py-8 text-[#f5efe0]/70">
                  {t("overseas.title")} - {t("common.loading")}
                </div>
                {/* {overseasCountriesData.map((countryData) => ({
                  name: t(`overseas.countries.${countryData.id}`),
                  image: countryData.image,
                  salary: countryData.salary[locale] || countryData.salary.zh,
                  hours: countryData.hours[locale] || countryData.hours.zh,
                  overtime: countryData.overtime[locale] || countryData.overtime.zh,
                  housing: countryData.housing[locale] || countryData.housing.zh,
                  religious: countryData.religious[locale] || countryData.religious.zh,
                })).map((country, index) => (
                  <Card
                    key={index}
                    className="group relative overflow-hidden bg-gradient-to-br from-[#2d1b40] to-[#1a0f2e] border-[#d4b96e]/20 hover:border-[#d4b96e]/40 transition-all duration-300"
                  >
                    <CardContent className="p-0 relative">
                      ...
                      <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
                        <div className="text-white space-y-3">
                          <h4 className="text-lg font-bold text-[#d4b96e] mb-3">
                            {country.name}
                          </h4>
                          ...
                          <div className="bg-black/30 rounded-lg p-3 space-y-2">
                            ...
                            <div className="text-xs space-y-1">
                              <div>
                                <span className="text-[#d4b96e]">{t("overseas.monthlySalary")}</span>{" "}
                                {country.salary}
                              </div>
                              <div>
                                <span className="text-[#d4b96e]">{t("overseas.workingHours")}</span>{" "}
                                {country.hours}
                              </div>
                              <div>
                                <span className="text-[#d4b96e]">{t("overseas.overtime")}</span>{" "}
                                {country.overtime}
                              </div>
                            </div>
                          </div>
