...
export const resetSessionCheckStatus = () => {
...
if (typeof window !== "undefined") {
  ...
  const lastInitTime = parseInt(
    sessionStorage.getItem("auth_init_time") || "0"
  );
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener(
        "reset-session-check",
        handleResetSessionCheck
      );
    };
  }, []);

  // 主会话检查逻辑 - 临时注释掉以禁用登录检查
  useEffect(() => {
    // 临时跳过所有会话检查逻辑
    console.log("[AuthSessionChecker] 会话检查已临时禁用");
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("auth-session-checked", {
          detail: { success: false, disabled: true },
        })
      );
    }
