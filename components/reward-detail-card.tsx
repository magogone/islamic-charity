...
export function RewardDetailCard({
  ...
  // 找到选中日期的奖励数据
  const selectedReward = selectedDay
    ? dailyRewards.find((reward) => reward.date === selectedDay)
    : null;

  // 计算差额
  const difference = selectedReward
    ? (selectedReward.maxReward - selectedReward.actualReward).toFixed(2)
    : "0.00";
  ...
  return (
    <div className="relative w-full rounded-xl bg-islamic-dark p-4 text-islamic-cream">
      ...
      <div className="flex justify-between text-xs text-islamic-cream/70 mb-2">
        <div>
          {t("rewardDetail.start")}: {startDate}
        </div>
        <div>
          {t("rewardDetail.end")}: {endDate}
        </div>
      </div>
          ...
          <div className="absolute top-0 left-1/2 transform -translate-x-1/2 bg-islamic-medium/95 backdrop-blur-sm p-4 rounded-lg border border-islamic-gold/30 w-[80%] max-w-[300px] z-10">
            <div className="flex justify-between items-center mb-2">
              <h4 className="text-lg font-bold text-islamic-gold">
                {selectedReward.date} {t("rewardDetail.rewardDetails")}
              </h4>
              <button
                className="text-islamic-cream/70 hover:text-islamic-cream"
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                <span className="text-islamic-cream/80">
                  {t("rewardDetail.actualReward")}:
                </span>
                <span className="font-medium text-islamic-gold">
                  {selectedReward.actualReward.toFixed(1)}{" "}
              ...
              <div className="flex justify-between">
                <span className="text-islamic-cream/80">
                  {t("rewardDetail.maxReward")}:
                </span>
                <span className="font-medium text-islamic-gold">
                  {selectedReward.maxReward.toFixed(1)}{" "}
                  <span className="text-xs">USD</span>
                </span>
              </div>
              ...
              <div className="flex justify-between">
                <span className="text-islamic-cream/80">
                  {t("rewardDetail.completionRate")}:
                </span>
                <span className="font-medium text-islamic-gold">
                  {selectedReward.completionRate}%
                </span>
              </div>

              <div className="flex justify-between">
                <span className="text-islamic-cream/80">
                  {t("rewardDetail.difference")}:
                </span>
                <span className="font-medium text-islamic-gold">
                  {difference} <span className="text-xs">USD</span>
                </span>
              </div>

              <div className="flex justify-between">
                <span className="text-islamic-cream/80">
                  {t("rewardDetail.status")}:
                </span>
                <span
